...
def get_last_records_geraet(geraet):
    ...
    if connection:
        try:
            with connection.cursor() as cursor:
                sql = f"SELECT * FROM {geraet} ORDER BY zeitstempel"#Annahme:'id'Primärschlüselspalt
                cursor.execute(sql)
                records = cursor.fetchall()
                data_list = list(records)

        except pymysql.Error as e:  # Diese Zeile könnte möglicherweise falsch eingerückt sein
...
last_records = get_last_records_geraet('geraet')
...
for record in last_records:
    ...
    record['zeitstempel'] = formatted_timestamp  # Aktualisieren des Zeitstempels im Dictionary
...
def get_last_records_sensor(sensor):
    """letzten datensätze speichern"""
    connection = connect_to_database()
    data_list = []
    ...
    if connection:
        try:
            with connection.cursor() as cursor:
                sql = f"SELECT * FROM {sensor} ORDER BY zeitstempel" #Annahme: 'id'
                cursor.execute(sql)
                records = cursor.fetchall()
                data_list = list(records)
...
last_records = get_last_records_sensor('sensor')
...
for record in last_records:
    ...
    # Direkt den Datensatz zur vorhandenen Liste 'daten_sensor' hinzufügen
    daten_sensor.append(record)
# daten mergen
def merge_data(var_daten_geraet, var_daten_sensor):
    """mergen der daten"""
    var_daten_komplett = []

    sensor_dict = {}
    for sensor_datensatz in var_daten_sensor:
    ...
    for geraet_datensatz in var_daten_geraet:
        ...
        var_daten_komplett.append(geraet_datensatz)

    return var_daten_komplett


daten_komplett = merge_data(daten_geraet, daten_sensor)
#print(daten_komplett)

def bearbeite_datensaetze(var_daten_geraet):
    """ Schritt 1: Zeilen löschen, wo geraet_id gleich 4 ist"""
    gesamt_energie_daten = {}
    for datensatz in var_daten_geraet:
